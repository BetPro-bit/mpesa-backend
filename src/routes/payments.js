...
router.post('/paybill/callback', async (req, res) => {
  try {
    ...
    await supabase.from('transactions').insert({
      user_id: user.id,
      amount,
      description: `Paybill Deposit (${TransID})`,
      status: 'completed',
      checkout_id: TransID,
      created_at: new Date()
    });
...
router.post('/paybill/check', auth, async (req, res) => {
  try {
    const { data: tx } = await supabase
      ...
      .eq('user_id', req.user.id)
      .eq('status', 'completed')
      .ilike('description', 'Paybill Deposit%')
